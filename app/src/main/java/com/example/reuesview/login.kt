    ...
    override fun onCreateView(
        ...
        button.setOnClickListener {
            ...
            if(msgUsrInput.trim().length>0&&passUsrInput.trim().length>0){
                ...
                val fragment_RecyclerView = Recycler_view().newInstance(usrInput.text.toString() , passInput.text.toString())
    ...
    private fun handleFacebookAccessToken(token : AccessToken) {
        ...
        firebaseAuth!!.signInWithCredential(credential).addOnCompleteListener { task ->
            if (task.isSuccessful) {
                ...
                val profile = Recycler_view().newInstance(user!!.photoUrl.toString(),user!!.displayName.toString())
                val fm = fragmentManager
                val transaction : FragmentTransaction = fm!!.beginTransaction()
